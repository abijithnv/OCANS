...
import 'package:df/predict_based_photo8.dart';

void main() {
  runApp(const ImageUploadSeven());
}

class ImageUploadSeven extends StatefulWidget{
  const ImageUploadSeven({Key? key}) : super(key: key);

  @override
  State<StatefulWidget> createState() {
    return _ImageUploadSeven();
  }
}

class _ImageUploadSeven extends State<ImageUploadSeven>{
  ...
  Future<void> uploadImage() async {
    ...
    try{
      ...
      var data = await http.post(Uri.parse("http://"+ip+":3000/upload_images7"),body: {"photo":base64Image,'uplid':uplid});
      ...
      if (status=="ok") {
        ...
        Navigator.push(
          context,
          MaterialPageRoute(builder: (context) => ImageUploadEight()),
        );
      }
      else{
        setState(() {

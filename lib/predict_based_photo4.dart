...
import 'package:df/predict_based_photo5.dart';

void main() {
  runApp(const ImageUploadFour());
}

class ImageUploadFour extends StatefulWidget{
  const ImageUploadFour({Key? key}) : super(key: key);

  @override
  State<StatefulWidget> createState() {
    return _ImageUploadFour();
  }
}

class _ImageUploadFour extends State<ImageUploadFour>{
  ...
  Future<void> uploadImage() async {
    ...
    try{
      ...
      var data = await http.post(Uri.parse("http://"+ip+":3000/upload_images4"),body: {"photo":base64Image,'uplid':uplid});
      ...
      if (status=="ok") {
        ...
        Navigator.push(
          context,
          MaterialPageRoute(builder: (context) => ImageUploadFive()),
        );

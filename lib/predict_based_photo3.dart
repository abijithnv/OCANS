...
import 'package:df/predict_based_photo4.dart';

void main() {
  runApp(const ImageUploadthree());
}

class ImageUploadthree extends StatefulWidget{
  const ImageUploadthree({Key? key}) : super(key: key);

  @override
  State<StatefulWidget> createState() {
    return _ImageUploadthree();
  }
}

class _ImageUploadthree extends State<ImageUploadthree>{
  ...
  Future<void> uploadImage() async {
    ...
    try{
      ...
      var data = await http.post(Uri.parse("http://"+ip+":3000/upload_images3"),body: {"photo":base64Image,'uplid':uplid});
      ...
      if (status=="ok") {
        ...
        Navigator.push(
          context,
          MaterialPageRoute(builder: (context) => ImageUploadFour()),
        );

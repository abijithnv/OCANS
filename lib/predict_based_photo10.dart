...
void main() {
  runApp(const ImageUploadTen());
}

class ImageUploadTen extends StatefulWidget{
  const ImageUploadTen({Key? key}) : super(key: key);

  @override
  State<StatefulWidget> createState() {
    return _ImageUploadTen();
  }
}

class _ImageUploadTen extends State<ImageUploadTen>{
  ...
  Future<void> uploadImage() async {
    ...
    try{
      ...
      var data = await http.post(Uri.parse("http://"+ip+":3000/upload_images10"),body: {"photo":base64Image,'uplid':uplid});
      ...
      if (status=="ok") {
        ...
        setState(() {
          Dname = "Result :" + jsondata['result'].toString();
          print("set");
        });

...
import 'package:df/predict_based_photo10.dart';

void main() {
  runApp(const ImageUploadNine());
}

class ImageUploadNine extends StatefulWidget{
  const ImageUploadNine({Key? key}) : super(key: key);

  @override
  State<StatefulWidget> createState() {
    return _ImageUploadNine();
  }
}

class _ImageUploadNine extends State<ImageUploadNine>{
  ...
  Future<void> uploadImage() async {
    ...
    try{
      ...
      var data = await http.post(Uri.parse("http://"+ip+":3000/upload_images9"),body: {"photo":base64Image,'uplid':uplid});
      ...
      if (status=="ok") {
        ...
        Navigator.push(
          context,
          MaterialPageRoute(builder: (context) => ImageUploadTen()),
        );
      }
      else{
        setState(() {

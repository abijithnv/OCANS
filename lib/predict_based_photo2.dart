...
void main() {
  runApp(const ImageUploadtwo());
}

class ImageUploadtwo extends StatefulWidget{
  const ImageUploadtwo({Key? key}) : super(key: key);

  @override
  State<StatefulWidget> createState() {
    return _ImageUploadtwo();
  }
}

class _ImageUploadtwo extends State<ImageUploadtwo>{
  ...
  Future<void> uploadImage() async {
    ...
    try{
      ...
      var data = await http.post(Uri.parse("http://"+ip+":3000/upload_images2"),body: {"photo":base64Image,'uplid':uplid});
      ...
      if (status=="ok") {
        ...
        Navigator.push(
          context,
          MaterialPageRoute(builder: (context) => ImageUploadthree()),
        );

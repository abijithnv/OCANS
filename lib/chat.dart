...
class MyChatApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      routes: {
        "/viewdoctors":(BuildContext context)=>new ViewUsers(),
      },
    );
  }
}
...
String name="";
  ...
  Future<void> view_message() async {
    ...
    try {
      ...
      String url="http://"+ip+":3000/and_viewchat";

      var data = await http.post(Uri.parse(url), body: {'from_id':pref.getString("lid").toString(),'to_id':pref.getString("dlid").toString()});
      ...
      for (int i = 0; i < arr.length; i++) {
        from_id.add(arr[i]['from_id']);
        to_id.add(arr[i]['to_id']);
                  ...
                  FloatingActionButton(
                    onPressed: () async {
                      ...
                      try {
                        ...
                        String url="http://"+ip+":3000/and_inschat";

                        var data = await http.post(Uri.parse(url), body: {'message':message,'from_id':pref.getString("lid").toString(),'to_id':pref.getString("dlid").toString()});
                        var jsondata = json.decode(data.body);
                        String status = jsondata['status'];

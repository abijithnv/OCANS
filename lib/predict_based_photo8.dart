...
import 'package:df/predict_based_photo9.dart';

void main() {
  runApp(const ImageUploadEight());
}

class ImageUploadEight extends StatefulWidget{
  const ImageUploadEight({Key? key}) : super(key: key);

  @override
  State<StatefulWidget> createState() {
    return _ImageUploadEight();
  }
}

class _ImageUploadEight extends State<ImageUploadEight>{
  ...
  Future<void> uploadImage() async {
    ...
    try{
      ...
      var data = await http.post(Uri.parse("http://"+ip+":3000/upload_images8"),body: {"photo":base64Image,'uplid':uplid});
      ...
      if (status=="ok") {
        ...
        Navigator.push(
          context,
          MaterialPageRoute(builder: (context) => ImageUploadNine()),
        );
      }
      else{
        setState(() {

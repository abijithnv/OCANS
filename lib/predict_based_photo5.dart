...
import 'package:df/predict_based_photo6.dart';

void main() {
  runApp(const ImageUploadFive());
}

class ImageUploadFive extends StatefulWidget{
  const ImageUploadFive({Key? key}) : super(key: key);

  @override
  State<StatefulWidget> createState() {
    return _ImageUploadFive();
  }
}

class _ImageUploadFive extends State<ImageUploadFive>{
  ...
  Future<void> uploadImage() async {
    ...
    try{
      ...
      var data = await http.post(Uri.parse("http://"+ip+":3000/upload_images5"),body: {"photo":base64Image,'uplid':uplid});
      ...
      if (status=="ok") {
        ...
        Navigator.push(
          context,
          MaterialPageRoute(builder: (context) => ImageUploadSix()),
        );
      }
      else{
        setState(() {
